...
- **День** (`view-habits` → `#day-normal`): `renderDayView()`/`renderDashboardHabits()`.
  ...
  `+ добавить привычку` (до 10) — инпут с `autocomplete="off"` + уникальным `name` (иначе браузер
  предлагает сохранённый email/логин в это поле — было замечено юзером). Настройки: `openHabitSettings`/`saveSettings` — модалка с
