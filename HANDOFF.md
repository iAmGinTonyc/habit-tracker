...
- **Месяц** (`renderMonthView`): тепловая карта (внутри клеток — сокр. день недели чёрным,
  ...
  **Мобильная адаптация тепловой карты (@media max-width:600px):** вместо сжатия всех ~30 дней в
  ...
  становится `flex-direction:row` (название слева, дни справа скроллятся); ширина клетки —
  `calc((100vw - 116px) / 7)` в `.hm-cells` (116px = 30 паддинг + 80 колонка названия + 6 зазор).
  Для ТЕКУЩЕГО месяца при рендере сразу `hm.scrollLeft` выставляется на позицию сегодня.
