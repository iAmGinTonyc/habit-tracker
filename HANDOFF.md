...
- **БЭКЕНД (Supabase) — В РАБОТЕ. Фаза 1 (аккаунты) ГОТОВА, см. §13.** Дальше:
  **Фаза 2 — синк** данных трекера (habits/checkins/foodLog/metricLog…) в облако под аккаунт;
  **Фаза 3 — семья**: приглашение по `invite_id`, входящие приглашения, просмотр статистики
  близких (только чтение). Перед Фазой 3 обсудить: что именно шарим, приватность, как принимают.
...
- **Архитектура:** статика (GitHub Pages) + Supabase напрямую с клиента, без билда/сервера.
  Клиент грузится с CDN (`esm.sh/@supabase/supabase-js@2`) в `auth.js` (ES-модуль), выставлен на `window.sb`.
- **Файлы:** [`auth.js`](auth.js) (клиент + UI входа), [`db/phase1_profiles.sql`](db/phase1_profiles.sql) (схема — выполнена в Supabase).
...
- **Аккаунт ОПЦИОНАЛЬНЫЙ:** трекер работает локально как раньше; вход добавляет облако/семью.
  Кнопка профиля в шапке (`#profile-btn`) → модалка `#auth-modal` (регистрация/вход email+пароль /
...
- **Тест-юзер** от проверки: `habitdemo.*@gmail.com` (можно удалить в Authentication → Users).
